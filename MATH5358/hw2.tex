...
\begin{enumerate}
  ...
  \item Determine the generating function for each of the following sequences:
   ...
   \uw{d}{1}, \uw{d}{2}... \uw{d}{n} where \uw{d}{i} \% 2 \eql 1
   
   1 and 3 each occur a nonzero, even number of times. 5, 7, and 9 can occur any amount of times.
   
   So:
   
   \eqn{(x^2 + x^4 + x^6 ...)^2(1 + x + x^2 ...)^3}
   which is
   \eqn{(\frac{1}{(1 - x)^2} - 1)^2(\frac{1}{1 - x})^3}
   \eqn{(\frac{1 - (1 - x)^2}{(1 - x)^2})^2(\frac{1}{1 - x})^3}
   \eqn{(\frac{2x - x^2}{(1 - x)^2})^2(\frac{1}{1 - x})^3}
   \eqn{\frac{(2x - x^2)^2}{(1 - x)^7}}
   \eqn{\frac{x^4 -4x^3 + 4x^2}{(1 - x)^7}}
   which decomposes into:
   \eqn{\frac{-1}{(x - 1)^3} + \frac{2}{(x - 1)^5} + \frac{-1}{(x - 1)^7}}
   \eqn{-\sum x^{3n} + 2\sum x^{5n} - \sum x^{7n} \lra \sum -x^{3n} + 2x^{5n} - x^{7n}}
   ...
   S(6, 6) \eql 6 S(5, 6) \ps S(5, 5)
   
   s(6, 6) \eql s(5, 5) \ms 5s(5, 6)
   
   \item Prove the Stirling numbers of the second kind satisfy:

   Recall: S(p, k) \eql k S(p \ms 1, k) \ps S(p \ms 1, k \ms 1) and
   \eqn{S(p, k) \eql \frac{1}{k!}\sum_{i = 0}^k(-1)^i\nck{k}{i}(k - i)^p}
   \balist
   \item S(n, 1) \eql 1
   ...
   Recall: s(p, k) \eql s(p \ms 1, k \ms 1) \ms (p \ms 1)s(p \ms 1, k) and
   \eqn{s(p, k) = \sum_{i \eql k}^p p^{i - k}s(p + 1, i + 1)}
