...
Q(n): \uw{s}{n} \ls 3 \fa n \mem \bn

\uw{s}{1} \eql 2 \ls 3

\uw{s}{2} \eql 2.25 \ls 3

Assume for k \mem \bn that \uw{s}{k} \ls 3

\eqn{s_{k} = \uf{\prn{1 \ps \frc{1}{k}}}{k} \ls 3}

\wts{\uw{s}{k + 1} \ls 3}

\eqn{\uf{\prn{1 \ps \frc{1}{k}}}{k} \ls 3}
\eqn{1 \ps \sum_{r = 1}^n \frac{1}{r!}\prod_i(1 - \frac{i}{n}) \ls 3}
\eqn{\fa n \mem \bn}

\eqn{\prod_i(1 - \frac{i}{n}) \ls 1}
\eqn{\textrm{(I think... I don't know where to go from here. How would you use } \frac{1}{1 - r}?)}

\wts{\prn{\uw{s}{n}} is convergent}

Since we just showed that \uw{s}{n} is increasing and bounded above (in theory), \uw{s}{n} is convergent by Theorem 4.3.3 (the Monotone Convergence Theorem)
